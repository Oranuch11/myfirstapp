...
function Counter() {
  ...
  return (
    <div>
      <span style={{ frontsize: "40px" }}>{count}</span>
  ...
  useEffect(
    function () {
      async function getFacts() {
        ...
        else alert("There was a problem data");
        setIsLoading(false);
      }
      getFacts();
    },
...
function NewFactForm({ setFacts, setShowForm }) {
  ...
  async function handleSumit(e) {
    //1.Orevent browser reload
    e.preventDefault();
    console.log(text, source, category);

    //2.Check if the data is vaild.If so, Create a new fact
    if (text && isValidHttpUrl(source) && category && textLength <= 200) {
      ...
      //3. Upload fact to supabase and receive the new facts
      setIsLoading(true);
  ...
  return (
    <form className="fact-form" onSubmit={handleSumit}>
      <input
        type="text"
        placeholder="Share a face with the world..."
        value={text}
